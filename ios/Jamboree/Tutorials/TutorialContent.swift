...
enum GameTutorials {
    ...
    static let cheat = GameTutorial(
        ...
        sections: [
            ...
            TutorialSection(heading: "Call bluff", body: "Any other player can call bluff on the last play. The cards flip — if the claim was a lie the cheater picks up the whole pile. If it was honest, the caller picks it up."),
            TutorialSection(heading: "Winning", body: "Empty your hand and survive the bluff window — if no-one calls (or the call fails), you win the round."),
        ],
        browserMenuSections: [
            ...
            TutorialSection(heading: "Calling bluff", body: "When someone else has just played, a red Call bluff button appears. Once the next player plays, the window closes — be quick."),
            TutorialSection(heading: "Pending win", body: "When someone plays their last card, the round pauses with Call bluff / Accept buttons. Catch the cheater or confirm the win."),
        ]
    )
